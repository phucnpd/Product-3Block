...
function show(number) {
    ...
    fetch("https://admin3blockapi.herokuapp.com/api/3block/system/getMessage", {
        method: "GET",
        headers: {
...
function appendData(data, length) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].isCheck == "true")
            text +=
                "<td><div class='checkbox123 checked'><input type='checkbox' id='check_04' checked disabled>" +
                "<label for='check_04'><span class='indicators' data-label-off='off' data-label-on='on'>" +
                "</span></label></div></td></tr>";
        else {
            text +=
                "<td><div class='checkbox123 checked'><input type='checkbox' id='check_04' onclick=checkMessage('" +
                data[i]._id +
                "')>" +
                "<label for='check_04'><span class='indicators' data-label-off='off' data-label-on='on'>" +
                "</span></label></div></td></tr>";
...
function appendDataSearch(data, length) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].isCheck == "true")
            ...
            text +=
                "<td><div class='checkbox123 checked'><input type='checkbox' id='check_04'" +
                " onclick=checkMessageSearch('" +
                data[i]._id +
                "')>" +
                "<label for='check_04'><span class='indicators' data-label-off='off' data-label-on='on'>" +
                "</span></label></div></td></tr>";
...
//         // API Create New URL
//         fetch(
//             "https://admin3blockapi.herokuapp.com/api/3block/system/postNotFound18",
//             {
//                 method: "POST",
...
function checkMessage(idMessage) {
    fetch(
        "https://admin3blockapi.herokuapp.com/api/3block/system/checkMessage",
        {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
